...
public class SaveLoadBase : MonoBehaviour {
	...
	protected Component 
		...
		stats, transData,
		menuUnit, unitProc;//removableCreator, 
	...
	protected void InitializeComponents()
	{
		print ("This version was released without being finished, and without the extensive three-month testing our previous versions went through." +
			...
			"\n- production between sessions (while the gamer is off-line);" +
...
//		soundFX = GameObject.Find ("SoundFX").GetComponent<SoundFX> ();
	...
	*/
	...
	{		
		...
		if (isBattleMap && structureClass != "Ambient") 
		{
			LoadedGrass.Add (grass);
	...
	protected void CalculateElapsedTime()
	{
		...
		if(!isBattleMap)
		{
			...
			{
				//((Stats)stats).gold += (int) ((existingBuildings[4]-finishTimesGold.Count) * ((Stats)stats).productionRates[0]*elapsedTime*60);
			}
			
			if (existingBuildings [5] - finishTimesMana.Count > 0) 
			{
				//((Stats)stats).mana += (int) ((existingBuildings[5] - finishTimesMana.Count) * ((Stats)stats).productionRates[1]*elapsedTime*60);
			}
			
			for (int i = 0; i < finishTimesGold.Count; i++) 
			{
				//((Stats)stats).gold += (int) (finishTimesGold[i]*((Stats)stats).productionRates[0]*60);
			}
			
			for (int i = 0; i < finishTimesMana.Count; i++) 
			{
				//((Stats)stats).mana += (int) (finishTimesMana[i]*((Stats)stats).productionRates[1]*60);
			}
	...
	{			
		...
		*/
		...
		if(!isBattleMap)
		{	
		((Stats)stats).structureIndex = int.Parse(statsTx[0]);
		((Stats)stats).experience = int.Parse(statsTx[1]);
		((Stats)stats).dobbits = int.Parse(statsTx[2]);
		((Stats)stats).occupiedDobbits = int.Parse(statsTx[3]);

		((Stats)stats).occupiedHousing = int.Parse(statsTx[4]);
		((Stats)stats).maxHousing = int.Parse(statsTx[5]);

		((Stats)stats).gold = int.Parse(statsTx[6]);
		((Stats)stats).mana = int.Parse(statsTx[7]);
		((Stats)stats).crystals = int.Parse(statsTx[8]);	

		((Stats)stats).maxGold = int.Parse(statsTx[9]);
		((Stats)stats).maxMana = int.Parse(statsTx[10]);
		((Stats)stats).maxCrystals = int.Parse(statsTx[11]);	
		...
		bool tutorialCitySeen = Boolean.Parse(statsTx [12]);//Convert.ToBoolean(statsTx [11]);
		if(!isBattleMap)
		...
		if (!isBattleMap) 
		{
			bool tutorialBattleSeen = Boolean.Parse (statsTx [13]);
			((Stats)stats).tutorialBattleSeen = tutorialBattleSeen;

			bool removablesCreated = Boolean.Parse (statsTx [14]);
			((Stats)stats).removablesCreated = removablesCreated;
		
			bool 
			soundOn = Boolean.Parse(statsTx [15]),
			ambientOn = Boolean.Parse(statsTx [16]),//ambient save/load next release
			musicOn = Boolean.Parse(statsTx [17]);
