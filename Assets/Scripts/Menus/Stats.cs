...
public class Stats : MonoBehaviour { //the resource bars on top of the screen
	...
	public int 	//when user hard buys resources, storage capacity permanently increases 
		...
		level = 1,						//level-1 to process the correct list
		experience = 1,
	...
	{
		string[] maxBuildingsString = (levels [level-1] ["MaxBuildings"]).Split("," [0]);
	...
	{		
		...
		string[] maxWallsString = (levels [level-1] ["MaxWalls"]).Split("," [0]);
	...
	{	
		string[] maxWeaponString = (levels [level-1] ["MaxWeapons"]).Split("," [0]);
	...
	{	
		string[] maxAmbientString = (levels [level-1] ["MaxAmbients"]).Split("," [0]);
