...
//		((Messenger)heliosMsg).DisplayMessage (text);
	...
	*/
	...
	{												   //this happends when the user builds and destroys buildings - the buildingIndex grows	
		...
		for (int i = 0; i < Buildings.Count; i++) 
		{
			if(Buildings[i].GetComponent<StructureSelector>().structureIndex == buildingIndex)
			{
				listIndex = i;
				break;
			}
	...
	private void UpdateCannons()
	{
		...
		for (int i = 0; i <= instantiationGroupIndex; i++) 
		{
			...
			if(GroupO.Count>0)
			{		
				Cannons[index].GetComponent<CannonControllerOld>().target = GroupO[0].transform.position;	
				Cannons[index].GetComponent<CannonControllerOld>().fire = true;
				keepFiring.Add(targetBuildingIndex[i]);
			}
			else
				Cannons[index].GetComponent<CannonControllerOld>().fire = false;
			break;

			case 1:
			if(GroupI.Count>0)
			{
				Cannons[index].GetComponent<CannonControllerOld>().target = GroupI[0].transform.position;	
				Cannons[index].GetComponent<CannonControllerOld>().fire = true;						
				keepFiring.Add(targetBuildingIndex[i]);
			}
			else
			...
			if(GroupII.Count>0)
			{
				Cannons[index].GetComponent<CannonControllerOld>().target = GroupII[0].transform.position;	
				Cannons[index].GetComponent<CannonControllerOld>().fire = true;
				keepFiring.Add(targetBuildingIndex[i]);
			}
			else
			...
			if(GroupIII.Count>0)
			{
				Cannons[index].GetComponent<CannonControllerOld>().target = GroupIII[0].transform.position;	
				Cannons[index].GetComponent<CannonControllerOld>().fire = true;
			}	
			else
			{					
...
//		((Relay)relay).pauseInput = true;
	...
	private void AllBuildingsIndestructible(bool b)
	{
		...
		for (int i = 0; i < BuildingsHealth.Count; i++) 
		{
			...
			//int index = GetBuildingListIndex (i);//GetBuildingListIndex(i);Buildings [i].GetComponent<StructureSelector> ().structureIndex

			//CreateorDestroyStar(index,b);//index
		}
	}
	...
	{
		int buildingIndex = Buildings[listIndex].GetComponent<StructureSelector>().structureIndex;
	...
	private void CreateBuildingStar(int index)
	{
		...
		Vector3 pos = Buildings[index].transform.position;//index

		GameObject BuildingStar =(GameObject)Instantiate(buildingStar, new Vector3(pos.x,pos.y-230,starZ), Quaternion.identity);	
		ProcessBuildingStar(BuildingStar, i);// index
	}

	private void ProcessBuildingStar(GameObject BuildingStar, int i)
	{		
